...
cachedGetBlogs("user-b") を /profile から呼んだ場合
  → どちらも同じ「user-b のキャッシュ」を読み書きする。どこから呼ばれたかは全く関係ない。
```

パスが違っても、関数と引数が同じなら**同じキャッシュエントリを共有します**。

### revalidatePathとは読み取り時に「再取得すべきか」を判定する仕組み
...
例えば`revalidatePath('/blogs')`をしたのちに/profileを訪れてもキャッシュエントリを上書きしません。つまり見た目は同じになります。
/blogsは予約されているのでキャッシュエントリを上書きします。次に/profileを訪れると先ほど上書きしたキャッシュエントリが帰ってきます。
前述したようにキャッシュエントリは必ず関数+引数のルールになっていることが重要です。
もしパス毎にキャッシュが存在していると理解している場合、上記の動作にはなりません。/blogsでキャッシュエントリが上書きされたあとに/profileに行っても変化がないはずです。キャッシュエントリが1つしかなく上書きする仕様だからこそこのような動きになるわけです。
